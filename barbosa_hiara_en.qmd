...
Thus, two concepts were defined for the duties' profiles: primary and secondary. Participants considered primary duties as those in which they were central figures in the execution of the activity (differentiating their actions from other school actors). On the other hand, secondary duties are the primary duties of different school actors, with the educational counselor providing support for their execution, as described in @tbl-barbosa-hiara-1-en.

```{r}
#| echo: false
tabela_1 <- tribble(
  ~Duties,              ~`Components of Duties`,                                                                                           ~`Type of Duty`,
  "Monitoring",         "Socioemotional, dual focus, classes",                                                                             NA,
  "Planning",           "Classes, pedagogical",                                                                                           NA,
  "Training",           "Socioemotional skills",                                                                                          NA,
  "Preparation",        "Activities, dual focus",                                                                                         NA,
  "Collective Actions", "Welcoming, circles, workshops, lectures, groups, talks",                                                         NA,
  "Care/Mediation",     "Mental, dialogue, reflection, welcoming, listening, educational support, projects, socioemotional, conversation circles, and dialogue circles", NA,
  "Observation",        "Classes",                                                                                                        NA,
  "Participation",      "Meetings, planning",                                                                                             NA,
  "Support",            "Socioemotional skills, pedagogical",                                                                             NA,
  "Meeting",            "Management team",                                                                                                NA
)
```

```{r}
#| label: tbl-barbosa-hiara-1-en
#| echo: false
#| tbl-cap: Profiles of Primary and Secondary Duties of Educational Advisors
tabela_1 |> 
  select(Duties, `Components of Duties`) |> 
  gt() |> 
    tab_row_group(
    label = md("**Secondary**"), 
    rows = 8:10
  ...
  tab_style(
    ...
    locations = cells_column_labels(columns = c(Duties,
...
tabela_2 |> 
  ...
  tab_style(
    ...
    locations = cells_column_labels(columns = c(Assignments,
                                                "Function",
                                                Objective))
    ) |> 
  tab_options(
...
Challenges consist of circumstances participants report about their daily practices that may hinder expected results or make it difficult to carry out interventions. For this analysis, we initially used data on individual challenges and challenges common to all educational counselors. We related them to the type of school, team involvement, and requested demands, as shown in @tbl-barbosa-hiara-3-en.

```{r}
tabela_3 <- tribble(
  ~`Participants’ Statements`, ~`Analysis Categories`,
  "Overload Pressure No day for planning Results must be quick “Solve it there” Last-minute demands",
  "Quality impairment regarding demand/role diversion",
  "Not having the same level of dedication Everything you do in one school, you have to do in the other “It’s like SEDUC deadlines” Develop the same work in schools Not respecting schedules Comparison I don’t believe there’s a tool that can handle it",
  "Split between two or more schools",
  "Help the school Help the community Feeling obligated Understaffed teams Being part of the management team, working nights and weekends",
  "Difficulty in managing activities beyond their duties"
)
```

```{r}
#| label: tbl-barbosa-hiara-3-en
#| tbl-cap: Challenges of Educational Counselors

tabela_3 |> 
  gt() |> 
  tab_row_group(
    label = "Guiding Concept of Analysis Categories: Highlights the difficulties faced, which lead to role diversion",
    rows = 3
  ) |> 
  tab_row_group(
    label = "Guiding Concept of Analysis Categories: Highlights the difficulties faced, which require demands for immediate results and equivalent actions",
    rows = 1:2
  ) |> 
  tab_options(
    table.font.size = px(12),
  ...
  tab_style(
    ...
    locations = cells_column_labels(columns = c("Participants’ Statements",
                                                "Analysis Categories"))
    ) |> 
  tab_style(
...
Although counselors mentioned the instruments, many reported using them infrequently in their daily work, and some never used them. This raises a question regarding how the work of educational counselors is quantified or whether the municipality has yet to consider data quantification necessary for counselors' work, as shown in @tbl-barbosa-hiara-4-en.

```{r}
tabela_4 <- tibble(
  Instruments = c(
    "Physical agenda Schedule Tracking table and class record form Telework plan",
    "IAS CSE repository",
    "Referral instrument Planning form Dual-focus activity record form Welcome form Attendance request",
    "Home visit record form Monthly activity map Dual-focus class monitoring Circle planning form"
  ),
  Adherence = c("Most used", "Least used", "Most used", "Least used"),
  `Guiding Concept` = c(
    "Explains how monthly quantitative reports of remote work by coordinators are conveyed",
    "Explains how monthly quantitative reports of remote work by coordinators are conveyed",
    "Explains how monthly quantitative reports of in-person work by coordinators are conveyed",
    "Explains how monthly quantitative reports of in-person work by coordinators are conveyed"
  )
)
```
...
```{r}
#| label: tbl-barbosa-hiara-4-en
#| tbl-cap: Systematization of Daily Instruments Used by Educational Counselors
tabela_4 |> 
  gt() |> 
  tab_options(
    table.font.size = px(12),
  ...
  tab_row_group(
    label = md("**Records of in-person activities – August 2019 to March 2020**"),
    rows = 3:4
  ) |> 
  tab_row_group(
    label = md("**Records of remote activities – March 2020 to April 2021**"),
    rows = 1:2
  ) |> 
  tab_style(
    style = list(
    ...
    locations = cells_column_labels(columns = c(Instruments,
                                                Adherence,
                                                "Guiding Concept"))
  )
```
...
When researching the professional profile and modes of operation of educational counselors in the municipal public school system of Sobral-CE, the first point that deserves attention is the academic training required by the municipal management for the position of educational counselor, since in much of the Brazilian territory, according to Barbosa and Marques [-@BarbosaMarques], there is a tendency to opt for pedagogues or other professionals with teaching degrees. Sobral, on the other hand, opted for psychologists.
...
```{r}
#| label: tbl-barbosa-hiara-5-en
#| tbl-cap: Effect Indicators
tabela_5 |> 
  gt() |> 
  tab_style(
    style = list(
    ...
    locations = cells_column_labels(columns = colunas <- c("Assignment", 
                                                           "Assignment Objectives", 
                                                           "Target Audience", 
                                                           "Effects Described by OE", 
                                                           "Suggested Effect Indicator/Measure"
    )
    )
  ) |> 
  tab_options(
