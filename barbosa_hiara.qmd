...
tabela_1 |> 
  ...
  tab_style(
    ...
    locations = cells_column_labels(columns = c(Atribuições,
                                                "Componentes das atribuições"))
    ) |>
  tab_options(
...
```{r}
#| label: tbl-barbosa-hiara-2
#| echo: false
#| tbl-cap: Funções das Atribuições do Orientador Educacional
tabela_2 <- tribble(
...
tabela_3 <- tribble(
  ...
  "Sobrecarga, Pressão, Sem dia para planejamento, Resultado tem que ser rápido, Resolve aí, Demandas de última hora",
  "Prejuízos na qualidade quanto à demanda/desvio de função",
  "Não ter o mesmo nível de dedicação, Tudo que faz em uma escola, tem que fazer na outra, Assemelhar-se aos prazos da SEDUC, Desenvolver o mesmo trabalho nas escolas, Não respeitar os horários, Comparar, Não acreditar que exista instrumento que dê conta",
  "Divisão em duas ou mais escolas",
  "Ajudar a escola, Ajudar a comunidade, Sentir-se na obrigação, Equipes desfalcadas, Ser do núcleo gestor, trabalhar noite e fim de semana",
  "Dificuldade na gestão das atividades fora de suas atribuições"
)
```


```{r}
#| label: tbl-barbosa-hiara-3
#| tbl-cap: Desafios dos Orientadores Educacionais

tabela_3 |> 
  gt() |> 
  tab_row_group(
    label = "Conceito norteador: salienta as dificuldades enfrentadas, as quais direcionam para um desvio de função",
    rows = 3
  ) |> 
  tab_row_group(
    label = "Conceito norteador: salienta as dificuldades enfrentadas, as quais exigem cobranças por resultados imediatos e ações equivalentes",
    rows = 1:2
  ) |> 
  tab_options(
    table.font.size = px(12),
  ...
  tab_style(
    ...
    locations = cells_column_labels(columns = c("Falas dos participantes",
...
tabela_4 <- tibble(
  Instrumentos = c(
    "Agenda física\nCronograma\nTabela de acompanhamento e ficha de registro das aulas\nPlano de teletrabalho",
    "Repositório das CSE do IAS",
    "Instrumental de encaminhamento\nFicha de planejamento\nFicha de registro das atividades\nFicha de acolhimento\nSolicitação de comparecimento",
    "Ficha Registro de visita domiciliar\nMapa mensal de atividades"
  ),
  Adesão = c("Mais utilizadas", "Menos utilizadas", "Mais utilizadas", "Menos utilizadas"),
  `Conceito norteador` = c(
    "Explicita a forma de repasse das produções mensais dos orientadores de caráter quantitativo do trabalho remoto",
    "Explicita a forma de repasse das produções mensais dos orientadores de caráter quantitativo do trabalho remoto",
    "Explicita a forma de repasse das produções mensais dos orientadores de caráter quantitativo do trabalho presencial",
    "Explicita a forma de repasse das produções mensais dos orientadores de caráter quantitativo do trabalho presencial"
  )
)
```
...
```{r}
#| label: tbl-barbosa-hiara-4
#| tbl-cap: Sistematização dos Instrumentais de Uso Diários dos Orientadores Educacionais
tabela_4 |> 
  gt() |> 
  tab_options(
    table.font.size = px(12),
  ...
  tab_row_group(
    label = md("**Registros de atividades presenciais – Agosto de 2019 a março de 2020**"),
    rows = 3:4
  ) |> 
  tab_row_group(
    label = md("**Registros de atividades remotas – Março de 2020 a abril de 2021**"),
    rows = 1:2
  ) |> 
  tab_style(
    style = list(
    ...
    locations = cells_column_labels(columns = c(Instrumentos,
                                                Adesão,
                                                "Conceito norteador"))
  )
```
...
```{r}
#| label: tbl-barbosa-hiara-5
#| tbl-cap: Indicadores de Efeitos
tabela_5 |> 
  gt() |> 
  tab_style(
    style = list(
    ...
    locations = cells_column_labels(columns = colunas <- c("Atribuição", 
                                                           "Objetivos das atribuicoes", 
                                                           "Publico-alvo", 
                                                           "Efeitos descritos pelos OE", 
                                                           "Sugestão de indicador de efeito medida"
    )
    )
  ) |> 
  tab_options(
